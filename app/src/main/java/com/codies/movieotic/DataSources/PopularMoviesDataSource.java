...
import com.codies.movieotic.NetworkUtils.PopularMovies;
...
public class PopularMoviesDataSource extends PageKeyedDataSource<Integer, Result> {
    ...
    public void loadInitial(@NonNull LoadInitialParams<Integer> params, @NonNull final LoadInitialCallback<Integer, Result> callback) {
        ...
        PopularMovies popularMovies = retrofit.create(PopularMovies.class);
        popularMovies.getMovies("381054badc0dabe1a386b89e1d169c2d", String.valueOf(firstPage))
    ...
    public void loadBefore(@NonNull final LoadParams<Integer> params, @NonNull final LoadCallback<Integer, Result> callback) {
        Retrofit retrofit = NetworkClient.getRetrofit();
        PopularMovies popularMovies = retrofit.create(PopularMovies.class);
        popularMovies.getMovies("381054badc0dabe1a386b89e1d169c2d", String.valueOf(params.key))
                .enqueue(new Callback<Example>() {
                    @Override
                    public void onResponse(Call<Example> call, Response<Example> response) {
                        if (response.body() != null) {
