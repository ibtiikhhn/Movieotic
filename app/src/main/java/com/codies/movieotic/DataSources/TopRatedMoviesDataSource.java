...
import com.codies.movieotic.NetworkUtils.TopRatedMovies;
...
public class TopRatedMoviesDataSource extends PageKeyedDataSource<Integer, Result> {
    ...
    public void loadInitial(@NonNull PageKeyedDataSource.LoadInitialParams<Integer> params, @NonNull final PageKeyedDataSource.LoadInitialCallback<Integer, Result> callback) {
        ...
        topRatedMovies.getMovies("381054badc0dabe1a386b89e1d169c2d", String.valueOf(firstPage))
    ...
    public void loadBefore(@NonNull final LoadParams<Integer> params, @NonNull final LoadCallback<Integer, Result> callback) {
        Retrofit retrofit = NetworkClient.getRetrofit();
        TopRatedMovies topRatedMovies = retrofit.create(TopRatedMovies.class);
        topRatedMovies.getMovies("381054badc0dabe1a386b89e1d169c2d", String.valueOf(params.key))
                .enqueue(new Callback<Example>() {
                    @Override
                    public void onResponse(Call<Example> call, Response<Example> response) {
                        if (response.body() != null) {
